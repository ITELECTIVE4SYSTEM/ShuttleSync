    ...
    if (isset($token_data['access_token'])) {
        ...
        if (isset($google_user['email'])) {
            ...
            if ($existing_user) {
                ...
                try {
                    $insert_stmt = $pdo->prepare("INSERT INTO Users (full_name, email, password, password_hash, role, skill_level) VALUES (:full_name, :email, :password, :password_hash, :role, :skill_level)");
                    $insert_stmt->execute([
                        'full_name' => $full_name, 'email' => $email, 'password' => $random_pass, 'password_hash' => $random_pass,
                    ...
                    error_log("Register (Google) DB error: " . $e->getMessage());
    ...
    if (empty($full_name) || empty($email) || empty($password)) {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            try {
                $insert_stmt = $pdo->prepare("INSERT INTO Users (full_name, email, password, password_hash, role, skill_level) VALUES (:full_name, :email, :password, :password_hash, :role, :skill_level)");
                $insert_stmt->execute([
                    'full_name' => $full_name, 'email' => $email, 'password' => $password_hash, 'password_hash' => $password_hash,
                    'role' => $role, 'skill_level' => $skill_level,
                ]);
                $_SESSION['user_id']   = $pdo->lastInsertId();
                $_SESSION['full_name'] = $full_name;
