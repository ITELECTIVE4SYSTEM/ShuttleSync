    ...
    if (isset($token_data['access_token'])) {
        ...
        if (isset($google_user['email'])) {
            ...
            if ($user) {
                ...
                $insert_stmt = $pdo->prepare("INSERT INTO Users (full_name, email, password, password_hash, role) VALUES (:name, :email, :passcol, :passhashcol, :role)");
                $insert_stmt->execute(['name' => $full_name, 'email' => $email, 'passcol' => $random_pass, 'passhashcol' => $random_pass, 'role' => $role]);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($login_input) || empty($password_input)) {
        ...
        $user = $stmt->fetch();
        if ($user && !empty($user['password_hash']) && password_verify($password_input, $user['password_hash'])) {
            $_SESSION['user_id']   = $user['user_id'];
            $_SESSION['full_name'] = $user['full_name'];
            $_SESSION['role']      = $user['role'];
            $_SESSION['logged_in'] = true;
