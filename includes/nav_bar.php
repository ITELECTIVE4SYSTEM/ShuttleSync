...
if (isset($pdo)) {
    try {
        ...
        // Read tracking: DB marker for logged-in users, session marker for everyone
        // (fallback that also covers guests).
        $db_last_read = null;
        if ($is_logged_in) {
            try {
                $read_stmt = $pdo->prepare("SELECT last_notif_read_at FROM Users WHERE user_id = ?");
                $read_stmt->execute([$_SESSION['user_id']]);
                $db_last_read = $read_stmt->fetchColumn();
            } catch (PDOException $e) {
                $db_last_read = null;
            }
        }
        $last_read = $db_last_read ?: ($_SESSION['notif_read_at'] ?? null);
        if ($last_read) {
            $unread = 0;
            foreach ($announcements as $ann) {
                if (strtotime($ann['created_at']) > strtotime($last_read)) $unread++;
            }
            $announcement_count = $unread;
        }
    } catch (PDOException $e) {
        $announcements = [];
    }
            ...
            <div class="relative">
                <button class="relative text-on-surface-variant hover:text-primary transition-all active:scale-95" id="notifButton" onclick="toggleNotifications(event)">
                    ...
                    <?php if ($announcement_count > 0): ?>
                        <span id="notifDot" class="absolute -top-1 -right-1 flex h-3 w-3">
                ...
                <div id="notifDropdown" class="absolute right-0 mt-2 w-80 bg-surface-container-lowest border border-outline-variant/50 rounded-xl shadow-xl overflow-hidden z-[60] origin-top-right hidden">
                    <div class="p-4 border-b border-outline-variant/30 bg-surface-bright flex justify-between items-center">
                        ...
                        <div class="flex items-center gap-2">
                            <span id="notifNewCount" class="<?php echo $announcement_count > 0 ? '' : 'hidden'; ?> text-[10px] bg-accent text-white px-2 py-0.5 rounded-full font-bold"><?php echo $announcement_count; ?> New</span>
                            <button type="button" id="markAllReadBtn" onclick="markNotificationsRead(true)" class="<?php echo $announcement_count > 0 ? '' : 'hidden'; ?> text-[11px] text-primary font-bold hover:underline">Mark all as read</button>
                        </div>
                    </div>
...
<script>
    ...
    function toggleNotifications(e) {
        ...
        if (wasHidden) markNotificationsRead(false);
    }
    ...
    function markNotificationsRead(showToast) {
        ...
        .then(d => {
            if (d && d.success) {
                ...
                const badge = document.getElementById('notifNewCount');
